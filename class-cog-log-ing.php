<?php
/**
 * cog-log-ing install, init, admin
 *
 * @package    cog-log-ing
 ...
 */
class Cog_Log_Ing {
	static $aws_config = null;
	static $cognito_client = null;
    ...
    public static function init() {
        error_log( 'Cog_Log_Ing::init' );
        if ( self::$initialized ) {
			return;
		}
        error_log( 'Cog_Log_Ing::init registering actions' );
	...
	public static function cognito_client( $aws_opts ) {
		if ( $aws_opts == self::$aws_config && self::$cognito_client != null ) {
	...
	public static function filter_protected_content( $content ) {
        ...
        if ( $user_name == "jonathan" && $password == "secret" ) {
